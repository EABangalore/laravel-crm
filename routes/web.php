...
Route::group(['middleware'=>'auth'],function(){
  ...
  Route::get('show_all_completed_jobs','newJobController@showAllCompletedJobs');
   ...
   Route::get('all_pending_jobs','newJobController@allPendingJobs'); 
    ...
    Route::get('customer-is-ready-for-quotation','newCustomerController@customerIsReadyForQuotation');  
     ...
     Route::post('edit_job/{id}','newJobController@editJob');  

     Route::get('filter_jobs','newJobController@filterJobs');

     Route::get('show_all_jobs','newJobController@showAllJobs');

     Route::post('hold_from_date','newCustomerController@holdFromDate');  


     Route::post('create-newjob','newCustomerController@createNewCustomer');

     Route::post('delete_job','newJobController@deleteJob');  


     Route::post('edit_customer/{id}','newCustomerController@editCustomer');  

     Route::post('original_job_post','newJobController@newJobPost');
...
Route::post('edit_quotation/{id}','quotationController@editQuotation');

Route::get('filter_quotation','quotationController@filterQuotations');
...
Route::get('encode_sent_id',function(){   
    ...
    $id = '2PslfYy';//$_POST['customer_id'];  
    $idd = decodeToBase64($id); 
...
/* from show_all_customers */
 ...
 Route::get('customer_filter_all','newCustomerController@customerFilterAll'); 

 Route::get('ready_for_quotation','newCustomerController@readyForQuotation');

 Route::get('hold_for_quotation','newCustomerController@holdForQuotation');

 Route::get('show_all_todays_customer','newCustomerController@showAllTodayCustomer');
...
/** -----------------------------------  Add supplier ----------------------------------- **/
  ...
  Route::get('supplier_for_a_project','add_supplier@supplierForAproject');  
/** -----------------------------------  Add supplier ----------------------------------- **/

/** ----------------------------- invoice related routes ----------------------------- **/
    Route::get('create_invoice','invoiceController@createInvoice');  // for performa inv
    ...
    Route::get('show_performa_table/{id}','invoiceController@showPerformaTable');
    Route::post('send_previewed_invoice/{id}','invoiceController@sendPreviewedInvoice'); 
    Route::post('send_previewed_invoice_to_customer/{id}','invoiceController@sendPreviewedInvoiceToCustomer'); 

   Route::get('filter_invoice','invoiceController@filterInvoice');  

   Route::post('set_performa_status/{id}','invoiceController@setPerformaStatus'); 

   Route::post('delete_performa_invoice/{id}','invoiceController@deletePerformaInvoice'); 

  Route::get('pending_performa_invoice','invoiceController@pendingPerformaInvoice');

  Route::get('sent_performa_invoice','invoiceController@sentPerformaInvoice');

  Route::get('follow_performa_invoice','invoiceController@followPerformaInvoice'); 

  Route::get('approved_performa_invoice','invoiceController@approvedPerformaInvoice'); 

  Route::get('delivery_note','invoiceController@createPerformaInvoice'); 
...
/* all Landing Routes */
   ...
   Route::get('performa_landing','allLandingPages@performaLanding');

/* all Landing Routes */

/* send user status */
   Route::post('send_customer_status','customerStatusController@sendCustomerStatus');
   Route::post('send_customer_quotation','customerStatusController@sendCustomerQuotation');
/* send user status */


Route::get('q',function(){
  $data = \App\products::orderBy('id','desc')->get();
  return view('test.quotation',['data'=>$data]);  
});
