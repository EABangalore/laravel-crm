	                 ...
	                 <div class="col-md-5"> 
						...
						<select name="customer_id" id="productIdentifier" class="form-control selectpicker" data-live-search="true">
						   ...
						   @foreach($data as $dat)
							   ...
							   <option  value="{{$dat->product_name}}" title="{{$dat->product_name}}" data-price="{{$dat->price}}" data-productdescription="{{$dat->productIdentifier}}">{{$dat->product_name}}</option>
          ...
          function showTotalAtBottom(){
           ...
           function calculateGstWithPredefinedValue(){
              ...
              $('#addCount').on('click',function(e){
                 ...
                 var tableBody = '<tr class="allTheQuotationRow"><td>'+productName+'</td><td contenteditable class="priceChangeField">'+price+'</td><td contenteditable class="quantityChangeField">'+quantity+'</td><td>'+amount+'</td><td><span class="label label-warning removeRow" style="width:20px;cursor:pointer;">cancel</span></td></tr>';
                ...
                $('#showQuotation').append('<tr id="lastTotalRow3333"><td></td><td></td><td></td><th>Total Amount</th><td></td></tr>');
